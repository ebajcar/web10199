...
function createBoard() {
    ...
    for (var i = 0; i < nor; i++) {
        ...
        for (var c = 0; c < noc; c++) {
            ...
            //newData.row = i;
            //newData.col = c; //save cell coordinates EXAMPLE ONLY (not used in this version) 
            if ((i * 5 + c) != 12) {
...
function populateCard() {
    ...
    for (var r = 0; r < nor; r++) {
        for (var c = 0; c < noc; c++) {
            ...
            // do...while is a statement that is executed at least once and 
...
function createCaller() {
    ...
    var targetEl = document.getElementsByTagName("footer")[0];
    // Get a reference to the parent element (<body> in this case)
    var parentDiv = targetEl.parentNode;
    // Insert the new element into the DOM before targetEl
    parentDiv.insertBefore(newEl, targetEl);
    parentDiv = newEl; // targetEl is the newly created div	
    ...
    for (var k = 1; k < 6; k++) {
        ...
        // using switch statement to run different code on each iteration
        switch (k) {
            case 1:
 ...
 */
function initAll() {
        // create the card (FACE: The individual bingo sheet 
        // containing 24 numbers and a free space in the middle.)
        createBoard();
        theCard = document.getElementsByTagName("td");
        // populate the card
        populateCard();
        // create the elements used by the caller to announce next number drawn
        createCaller();
    } // end of initAll() function	
...
document.getElementById("next").addEventListener("click", function() {
    ...
    // not implemented - the index does not correspond to the index (property)
    ...
    //check if number on the card, if yes treat same as user clicking an  unused cell
    for (var i = 0; i < 25; i++) {
